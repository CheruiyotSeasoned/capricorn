...
export function LoanProgressPanel({ status, appliedAmount }: { status: string; appliedAmount: number }) {
  ...
  return (
    <div className="rounded-lg border bg-white p-5 shadow-md dark:bg-gray-900 mt-6 space-y-4">
      ...
      <div className="p-3 bg-gray-50 dark:bg-gray-800 border rounded">
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Loan Status:</p>
      ...
      <div className="w-full flex flex-col space-y-2">
        {renderNextAction(status, depositAmount)}
      </div>
    </div>
  );
}
      ...
      {steps.map((step, index) => (
        <div key={index} className="flex-1 flex flex-col items-center">
          <div
            ...
            ${index < currentStep ? "bg-green-600 text-white" : "bg-gray-300 text-gray-600 dark:bg-gray-700 dark:text-gray-300"}`}
          >
            {index + 1}
          </div>
          <p className="mt-2 text-xs text-center text-gray-700 dark:text-gray-300">{step}</p>
        </div>
      ))}
    </div>
  );
...
function renderNextAction(status: string, depositAmount: number) {
  switch (status) {
    case "approved":
    case "deposit_pending":
      return <DepositInstructionsModal amount={depositAmount} />;
...
function mapStatusToStep(status: string): number {
  switch (status) {
    ...
    case "deposit_pending": return 3;
    case "deposit_received": return 3; // or 4 if funds ready
