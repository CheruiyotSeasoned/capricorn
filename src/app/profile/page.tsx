...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const fetchLoanOverview = async () => {
      try {
        const res = await fetch("/api/loans/overview", {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        const result = await res.json();
        console.log("LoanOverview result:", result);

        if (result.success && result.data) {
          setLoanOverview(result.data);
        }
      } catch (err) {
        console.error("Failed to load loan overview:", err);
      }
    };
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 rounded-lg bg-gray-100 dark:bg-gray-800 px-6 py-4 text-lg font-semibold text-primary dark:text-white shadow-sm inline-block">
              Ksh{" "}
              {loanOverview?.latestLoan?.securityAmount
                ? loanOverview.latestLoan.securityAmount.toLocaleString("en-KE", {
          ...
          <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="rounded-lg bg-blue-100 dark:bg-blue-900 px-6 py-4 text-center">
              ...
              <p className="text-xl font-bold text-blue-700 dark:text-blue-300">
                Ksh{" "}
                {loanOverview?.latestLoan?.appliedAmount
                  ? loanOverview.latestLoan.appliedAmount.toLocaleString("en-KE", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })
                  : "0.00"}
            ...
            <div className="rounded-lg bg-green-100 dark:bg-green-900 px-6 py-4 text-center">
              ...
              <p className="text-xl font-bold text-green-700 dark:text-green-300">
                Ksh{" "}
                {loanOverview?.latestLoan?.qualifiedAmount
                  ? loanOverview.latestLoan.qualifiedAmount.toLocaleString("en-KE", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })
                  : "0.00"}
